...
class ServiceLogic
{
    ...
    {
        return Service::active()->with(['rating'])->where('id', $id)->first();
    }

    public static function get_latest_products($limit, $offset, $vendor_id, $category_id, $type)
    {
        $paginator = Service::active()->type($type);
        if($category_id != 0)
        {
        ...
        $paginator = $paginator->with(['rating'])->where('vendor_id', $vendor_id)->latest()->paginate($limit, ['*'], 'page', $offset);
    ...
    public static function get_related_products($service_id)
    {
        $service = Service::find($service_id);
        return Service::active()->with(['rating'])
        ->whereHas('vendor', function($query){
            // $query->Weekday();
        })
        ->where('category_ids', $service->category_ids)
        ->where('id', '!=', $service->id)
        ->limit(10)
        ->get();
    }
    ...
    {
        $key = explode(' ', $name);
        $paginator = Service::active()->with(['rating'])->whereHas('vendor', function($q)use($zone_id){
    ...
    public static function popular_products($zone_id, $limit = null, $offset = null, $type)
    {
        ...
        {
            $paginator = Service::with(['rating'])->whereHas('vendor', function($q)use($zone_id){
                $q->where('zone_id', $zone_id);//->Weekday();
        ...
        $paginator = Service::active()->type($type)->with(['rating'])->whereHas('vendor', function($q)use($zone_id){
            $q->where('zone_id', $zone_id);//->Weekday();
