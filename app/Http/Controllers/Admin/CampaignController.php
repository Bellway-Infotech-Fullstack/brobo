...
class CampaignController extends Controller
{
    ...
    public function storeItem(Request $request)
    {
        ...
        $campaign->add_ons = json_encode([]);
        $campaign->vendor_id = Service::find($request->service_id)->vendor_id;
    ...
    {   
        ...
        {
            $campaign = Campaign::findOrFail($campaign);
            $vendors = $campaign->vendors()->paginate(config('default_pagination'));
            $vendor_ids = []; 
            foreach($campaign->vendors as $restaurant)
            {
                $vendor_ids[] = $restaurant->id;
            }
            return view('admin-views.campaign.basic.view', compact('campaign', 'vendors', 'vendor_ids'));
        }
        else
        {
            $campaign = ItemCampaign::findOrFail($campaign);
    ...
    {
        $campaign->vendors()->detach($restaurant);
        $campaign->save();
    ...
    {
        $campaign->vendors()->attach($request->vendor_id);
        $campaign->save();
