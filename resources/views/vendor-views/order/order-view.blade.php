                ...
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <div class="d-sm-flex align-items-sm-center">
                        ...
                        @elseif($order['order_status']=='accepted')
                            <span class="badge badge-soft-info ml-2 ml-sm-3 text-capitalize">
                              <span class="legend-indicator bg-info"></span>{{__('messages.accepted')}}
                            </span>
                    ...
                    <div class="mt-2">
                       ...
                       {{--  @php($order_delivery_verification = (boolean)\App\Models\BusinessSetting::where(['key' => 'order_delivery_verification'])->first()->value) --}}
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                    <a class="dropdown-item {{$order['order_status']=='pending'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'pending'])}}','Change status to pending ?')"
                                       href="javascript:">{{__('messages.pending')}}</a>
                                    <a class="dropdown-item {{$order['order_status']=='accepted'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'accepted'])}}','Change status to accepted ?')"
                                       href="javascript:">{{__('messages.accepted')}}</a>
                                    <a class="dropdown-item {{$order['order_status']=='processing'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'processing'])}}','Change status to processing ?')"
                                       href="javascript:">{{__('messages.processing')}}</a>
                                   {{--  <a class="dropdown-item {{$order['order_status']=='handover'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'handover'])}}','Change status to handover ?')"
                                       href="javascript:">{{__('messages.handover')}}</a> --}}
                                    <a class="dropdown-item {{$order['order_status']=='services_ongoing'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'services_ongoing'])}}&otp=' + document.getElementById('otp').value,'Change status to Service ongoing?')"
                                       href="javascript:">{{__('messages.serviceOngoing')}}</a>
                                    <a class="dropdown-item {{$order['order_status']=='delivered'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','Change status to completed (payment status will be paid if not)?')"
                                       href="javascript:">{{__('messages.delivered')}}</a>
                                    <a class="dropdown-item {{$order['order_status']=='canceled'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'canceled'])}}','Change status to canceled ?')"
                        ...
                        {{-- <div class="hs-unfold float-right">
                            @if($order['order_status']=='pending')
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'accepted'])}}','Change status to accepted ?')" href="javascript:">Accept this booking</a>
                            ...
                            @endif
                            @elseif ($order['order_status']=='accepted')
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'processing'])}}','Change status to processing ?')" href="javascript:">Change to Processing</a>

                            @elseif ($order['order_status']=='processing')
                            <input type="text" id='otp' class="form-control-inline form-control-sm" placeholder="Customer OTP" />
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'services_ongoing'])}}&otp=' + document.getElementById('otp').value,'Change status to Service Ongoing ?')" href="javascript:">Ready for Service</a>  

                            @elseif ($order['order_status']=='services_ongoing')
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','Change status to completed ?')" href="javascript:">Change status to Completed</a>--}}    
                            {{-- @elseif ($order['order_status']=='ser' && ($order['order_type']=='take_away' || \App\CentralLogics\Helpers::get_restaurant_data()->self_delivery_system))
                ...
                <div class="card mb-3 mb-lg-5">
                    ...
                    @foreach($order->details as $key=>$detail)
                        @if($detail->service )
                            <!-- Media -->
                            <div class="media">
                                <a class="avatar avatar-xl mr-3 cursor-pointer" href="{{route('vendor.service.view',$detail->service['id'])}}">
                                    <img class="img-fluid"
                                ...
                                <div class="media-body">
                                    <div class="row">
                                        <div class="col-md-6 mb-3 mb-md-0">
                                            <strong> {{$detail->service['name']}}</strong><br>
                                        ...
                                        <div class="col col-md-2 align-self-center">
                                            <h6>{{\App\CentralLogics\Helpers::format_currency($detail->service->price)}}</h6>
                                        </div>
                        ...
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-sm-right">
                                    ...
                                    <dd class="col-sm-6">{{\App\CentralLogics\Helpers::format_currency($product_price+$total_tax_amount+$total_addon_price-$coupon_discount_amount - $restaurant_discount_amount)}}</dd>
                                </dl>
                                <!-- End Row -->
                            </div>
                        </div>
                    ...
                    @if($order->customer)
                        <div class="card-body">
                            ...
                            <div class="media align-items-center" href="javascript:">
                                ...
                                <div class="media-body">
                                    <span class="text-body text-hover-primary">{{\App\Models\Order::where('user_id',$order['user_id'])->count()}} bookings</span>
                                </div>
                                <div class="media-body text-right">
                                    {{--<i class="tio-chevron-right text-body"></i>--}}
                                </div>
