              ...
              <div class="col-md-12 my-3">
                <div class="card">
                    <div class="card-header">
                        <h3 class="h3 mb-0 text-capitalize">{{__('messages.kyc_details')}} {{__('messages.details')}} </h3>
                    </div>
                    <div class="card-body">
                        <div class="col-md-8 mt-4">
                            <h4>Aadhaar Card Number
                                : {{auth('vendor')->user()->aadhaar_card_number ? auth('vendor')->user()->aadhaar_card_number : 'No Data found'}}</h4>

                            <h6>Pan Card Number : 
                                {{auth('vendor')->user()->pan_card_number ? auth('vendor')->user()->pan_card_number : 'No Data found'}}</h6>

                           {{--  <h6>KYC : 
                                {{auth('vendor')->user()->kyc ? auth('vendor')->user()->kyc : 'No Data found'}}</h6> --}}

                            <a class="btn btn-primary"
                               href="{{route('vendor.profile.kycEdit')}}">{{__('messages.edit')}}</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="col-md-8 mt-4">
                            ...
                            <h6>IFSC Code
                                : {{$data->ifsc_code ? $data->ifsc_code : 'No Data found'}}</h6>
