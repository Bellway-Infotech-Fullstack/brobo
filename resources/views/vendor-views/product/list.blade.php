        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-filter-list"></i> {{__('messages.service')}} {{__('messages.list')}}<span class="badge badge-soft-dark ml-2" id="itemCount">{{$foods->total()}}</span></h1>
                </div>
                ...
                <div class="col-sm-auto">
                    <a href="{{route('vendor.service.add-new')}}" class="btn btn-primary pull-right"><i
                        ...
                        <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                            ...
                            @foreach($foods as $key=>$food)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td>
                                        <a class="media align-items-center" href="{{route('vendor.service.view',[$food['id']])}}">
                                    ...
                                    <td>
                                        <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$food->id}}">
                                            <input type="checkbox" onclick="location.href='{{route('vendor.service.status',[$food['id'],$food->status?0:1])}}'"class="toggle-switch-input" id="stocksCheckbox{{$food->id}}" {{$food->status?'checked':''}}>
                                            <span class="toggle-switch-label">
                                                <span class="toggle-switch-indicator"></span>
                                            </span>
                                        </label>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-white"
                                            href="{{route('vendor.service.edit',[$food['id']])}}" title="{{__('messages.edit')}} {{__('messages.service')}}"><i class="tio-edit"></i>
                                        </a>
                                        <a class="btn btn-sm btn-white" href="javascript:"
                                            onclick="form_alert('food-{{$food['id']}}','Want to delete this item ?')" title="{{__('messages.delete')}} {{__('messages.service')}}"><i class="tio-delete-outlined"></i>
                                        </a>
                                        <form action="{{route('vendor.service.delete',[$food['id']])}}"
...
@push('script_2')
    ...
    <script>
        $('#search-form').on('submit', function (e) {
            ...
            $.post({
                url: '{{route('vendor.service.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
