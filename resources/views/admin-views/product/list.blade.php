        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"> {{__('messages.service')}} {{__('messages.list')}}<span class="badge badge-soft-dark ml-2" id="foodCount">{{$services->total()}}</span></h1>
                </div>
                ...
                <div class="col-sm-auto" style="min-width: 306px;">
                    <select name="restaurant_id" id="restaurant" onchange="set_restaurant_filter('{{url()->full()}}',this.value)" data-placeholder="{{__('messages.select')}} {{__('messages.vendor')}}" class="js-data-example-ajax form-control" onchange="getRestaurantData('{{url('/')}}/admin/vendor/get-addons?data[]=0&restaurant_id=',this.value,'add_on')" required title="Select Vendor" oninvalid="this.setCustomValidity('{{__('messages.please_select_restaurant')}}')">
                    @if($restaurant)    
                    <option value="{{$restaurant->id}}" selected>{{$restaurant->name}}</option>
                    @else
                    <option value="all" selected>{{__('messages.all_vendors')}}</option>
                    @endif
                    </select>
                </div>
            </div>
                    ...
                    <div class="table-responsive datatable-custom" id="table-div">
                        <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                            ...
                            <tr>
                                ...
                                <th style="width: 15%">{{__('messages.vendor')}}</th>
                            ...
                            <tbody id="set-rows">
                            @foreach($services as $key=>$food)
                                <tr>
                                    <td>{{$key+$services->firstItem()}}</td>
                                    <td>
                                        <a class="media align-items-center" href="{{route('admin.service.view',[$food['id']])}}">
                                    ...
                                    <td>
                                    {{$food->vendor?$food->vendor->names():__('messages.vendor').' '.__('messages.deleted')}}
                                    </td>
                                    ...
                                    <td>
                                        <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$food->id}}">
                                            <input type="checkbox" onclick="location.href='{{route('admin.service.status',[$food['id'],$food->status?0:1])}}'"class="toggle-switch-input" id="stocksCheckbox{{$food->id}}" {{$food->status?'checked':''}}>
                                            <span class="toggle-switch-label">
                                                <span class="toggle-switch-indicator"></span>
                                            </span>
                                        </label>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-white"
                                            href="{{route('admin.service.edit',[$food['id']])}}" title="{{__('messages.edit')}} {{__('messages.service')}}"><i class="tio-edit"></i>
                                        </a>
                                        <a class="btn btn-sm btn-white" href="javascript:"
                                            onclick="form_alert('food-{{$food['id']}}','{{__('messages.Want_to_delete_this_item')}}')" title="{{__('messages.delete')}} {{__('messages.service')}}"><i class="tio-delete-outlined"></i>
                                        </a>
                                        <form action="{{route('admin.service.delete',[$food['id']])}}"
