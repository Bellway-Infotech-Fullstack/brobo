            ...
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <form action="javascript:" method="post" id="campaign_form"
                    ...
                    <div class="row">
                                    ...
                                    @foreach($zones as $zone)
                                        @if(isset(auth('admin')->user()->zone_id))
                                            @if(auth('admin')->user()->zone_id == $zone->id)
                                                <option value="{{$zone->id}}" {{$campaign->vendor->zone_id == $zone->id? 'selected': ''}}>{{$zone->name}}</option>
                                            @endif
                                        @else
                                            <option value="{{$zone->id}}" {{$campaign->vendor->zone_id == $zone->id? 'selected': ''}}>{{$zone->name}}</option>
                                        @endif
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="service_id" class="js-data-example-ajax form-control" onchange="getRestaurantData('{{url('/')}}/admin/vendor/get-addons?data[]=0&service_id='+this.value,'add_on')"  title="Select Service" required>
                                    @if($campaign->service)
                                    <option value="{{$campaign->service->id}}" selected>{{$campaign->service->name}}</option>
                                    @else
                  ...
                  {{--   <div class="row mt-2">
                    ...
                    <div class="form-group">
                        <label>{{__('messages.campaign')}} {{__('messages.image')}}</label><small style="color: red">* ( {{__('messages.ratio')}} 1:1 )</small>
            ...
            $.post({
                ...
                success: function (data) {
                    if (data.errors) {
                        ...
                        setTimeout(function () {
                            location.href = '{{route('admin.campaign.list', 'service_campaigns')}}';
                        }, 2000);
                    }
                }
            });
